...
if (await windowsSignatureFile.exists()) {
    updaterMeta.platforms["windows-x86_64"].signature = await windowsSignatureFile.text();
    updaterMeta.platforms["windows-x86_64"].url =
        `https://github.com/savannstm/rpgmtranslate/releases/download/${version}/rpgmtranslate_${version}_x64_en-US.msi`;
}

if (await linuxSignatureFile.exists()) {
    updaterMeta.platforms["linux-x86_64"].signature = await linuxSignatureFile.text();
    updaterMeta.platforms["linux-x86_64"].url =
        `https://github.com/savannstm/rpgmtranslate/releases/download/${version}/rpgmtranslate_${version}_amd64.AppImage`;
}

await Bun.write("./src-tauri/target/release/bundle/updater_info.json", JSON.stringify(updaterMeta));
